...
generated ~15,000-player database — and lets you start a career as any club and browse your
real, generated squad. See `fable-plans/plan1.md` for the full build plan and milestone list
(this is M0–M2 of that plan; calendar/match-day/transfers/etc. are still ahead).
...
milestones — calendar, transfers, scouting, etc.): Central's news/table, Transfers, Office,
Season. These render the same placeholder content the original prototype always had.
...
| **Central** | Advance day-strip (real calendar date), news headline, Global Transfer Network, News, Tables *(news/table content is still placeholder)* |
...
| **Office** | Inbox, Contracts, Youth Staff, My Career, Request Funds, Browse Jobs, Settings *(placeholder content)* |
| **Season** | Calendar, Other Leagues, Team/Player Stats, Tables, Fixtures *(placeholder content)* |
...
- `dev/world.html` — browses the authored world data (`data/*.json`): every league's clubs
  ...
  nations.
- `dev/tests.html` — live assert harness verifying milestone M2 (player/world generation):
  the mandatory overall-calculation test vectors (Messi → 93±1, Neuer → 88), growth-curve
  sanity, a full world generation with a performance budget (<3s), RNG determinism, and a
  compact-save serialization round-trip. Should read **40/40 passing**.
...
js/ui/render.js              # renders Central/Squad/Transfers/Season/email from GameState
...
dev/tests.html, dev/tests.js # dev page: assert harness for player/world generation
